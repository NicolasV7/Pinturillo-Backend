...
module.exports = (expressWs) => {
  ...
  router.ws("/game/:roomId", async (ws, req) => {
    ...
    ws.on("message", async (msg) => {
      ...
      else if(jsonMessage.type === "START_GAME" && SocketController.rooms[idRoom].size > 1 && room.state === 'En curso') {
        await startGame();
      }
    });
        ...
        const userPromise = users.map(async (user: any) => {
          ...
          return new Promise<void>((resolve) => {
            const interval = setInterval(() => {
              timeController--;
              time = timeController;
              if(timeController > 0 && solveWord.length < SocketController.rooms[idRoom].size) {
